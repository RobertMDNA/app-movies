Vue.component('props-comp', { // componente props-comp global
    template: `
        <div>
            <h1>Peliculas Props</h1>
            <MovieComp v-for="(movie, key) in movies" :key="key" :id="movie.id" :title="movie.title" :synopsis="movie.synopsis" :cover="movie.cover"/> <!-- pasando data entre componentes -->
        </div>
    `,
    data(){
        return{
